"""
Model C: Graph Encoder (SIREN message passing + Advanced Positional Aware Identity) 
Input  = Feynman diagram as PyG graph
Output = tokenised squared-amplitude

Upgrades implemented here:
1. Absolute Node Index Embeddings (solves permutation invariance)
2. Random-Walk Positional Encodings (RWSE topology awareness)
3. SIREN Message Passing layer directly ingests Edge Physics Attributes
"""
...
class SirenMLP(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, omega=30.0):
        super().__init__()
        self.net = nn.Sequential(
            SirenLinear(in_dim, hidden_dim, omega=omega, is_first=True),
            SirenLinear(hidden_dim, hidden_dim, omega=omega),
...
class SIRENMessagePassingLayer(MessagePassing):
    def __init__(self, d, edge_dim, omega=30.0, dropout=0.1):
        super().__init__(aggr='add')
        # Ingests source node (d), target node (d), and edge attributes (edge_dim)
        self.msg_siren = SirenMLP(d * 2 + edge_dim, d * 2, d, omega=omega)
        self.upd_mlp   = nn.Sequential(
            nn.Linear(d * 2, d), nn.ReLU(), nn.LayerNorm(d)
        )
    ...
    def message(self, x_i, x_j, edge_attr):
        # Concatenate edge_attr effectively forcing KANs to evaluate Physics Momentum directly
        return self.drop(self.msg_siren(torch.cat([x_i, x_j, edge_attr], dim=-1)))

    def update(self, aggr_out, x):
        return self.upd_mlp(torch.cat([x, aggr_out], dim=-1))

class AdvancedGraphSIRENEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # 3. SIREN convolutions
        self.convs  = nn.ModuleList([
            SIRENMessagePassingLayer(GNN_HIDDEN, EDGE_FEAT_DIM, SIREN_OMEGA, DROPOUT)
...
class AdvancedGraphSIRENTransformer(nn.Module):
    def __init__(self, tgt_vocab_size):
        super().__init__()
        self.encoder  = AdvancedGraphSIRENEncoder()
...
def train_domain(domain_name, data_dir, save_dir):
    print(f'\n{"="*60}')
    print(f'  Advanced Graph(SIREN) + Transformer — {domain_name}')
    print(f'{"="*60}')
    ...
    model = AdvancedGraphSIRENTransformer(len(vocab)).to(DEVICE)
    ...
    ckpt = save_dir / f'advanced_graph_siren_{domain_name.lower()}_best.pt'
...
if __name__ == '__main__':
    ...
    print('\n' + '='*60)
    print('  Advanced Graph(SIREN) + Transformer — Final Results')
    print('='*60)
